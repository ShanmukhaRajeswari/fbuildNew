...
//******************************************************************************************************
       ...
       router.get("/serviceprovider") { _, response, next in
            defer {
                next()
            }
        ...
        let result = try users.find(["status": "0"])
                  var inter = [[String: Any]]()
                  for x in result{
                  inter.append(["name":x["Name"]!,"phonenumber":x["phoneNumber"]!]) 
            }
        let context:[String:Any] = [
            "sps":inter
        ]  
        print(context)               
     try response.render("service_provider", context: context).end()               
...
//complete details of sp******************************************************************
       ...
       router.get("/specificSp") { _, response, next in
            defer {
                next()
            }
        ...
        let database = server["fbuild"]
        let users = database["serviceprovider"]
        let result = try users.find(["Email": "sodales.Mauris.blandit@tinciduntadipiscing.com"])
                  var inter = [[String: Any]]()
                  for x in result{
                  inter.append(["name":x["Name"]!,"phonenumber":x["PhoneNumber"]!,"email":x["Email"]!,"image":x["Image"]!,"TypeOfService":x["TypeOfService"]!]) 
            }
        let context:[String:Any] = [
            "sspd":inter
        ]  
...
// my team*********************************************************************************
     ...
     router.get("/myTeam") { _, response, next in
        ...
        let result = try users1.find(["U_email": "durgadevichaganti457@gmail.com"])
                  ...
                  for y in result2{
                  inter2.append(["provider_name": y["Name"]!, "PhoneNumber": y["PhoneNumber"]!, "TypeOfService": y["TypeOfService"]!])
                  }
            }
        let context:[String:Any] = [
            "myteam":inter2
...
//*******************************************************reviews/feedback
 ...
 router.get("/reviews") { _, response, next in
            defer {
                next()
            }
        ...
        let result = try users1.find(["U_email": "durgadevichaganti457@gmail.com"])
                  ...
                  var result2 = try users2.find(["Email": member_email])
                  for y in result2{
                  inter2.append(["provider_name": y["Name"]!, "PhoneNumber": y["PhoneNumber"]!, "TypeOfService": y["TypeOfService"]!, "Review": Review!])
                  }
            }
        let context:[String:Any] = [
            "reviews":inter2
        ...
        print(context)               
     try response.render("reviews", context: context).end()               
...
//*****************Current Projects**********************
       ...
       router.get("/currentProjects") { _, response, next in
        ...
        let result = try users.find(["U_email": "durgadevichaganti457@gmail.com", "P_Status": "0"])
                  var inter = [[String: Any]]()
                  for x in result{
                  inter.append(["project_name": x["Name"]!,"Type": x["Type"]!]) 
            }
        let context:[String:Any] = [
...
//*****************Previous Projects**********************
       ...
       router.get("/previousProjects") { _, response, next in
        ...
        let database = server["fbuild"]
        let users = database["projects"]
        let result = try users.find(["U_email": "durgadevichaganti457@gmail.com", "P_Status": "1"])
                  var inter = [[String: Any]]()
                  for x in result{
                  inter.append(["project_name": x["Name"]!,"Type": x["Type"]!]) 
            }
